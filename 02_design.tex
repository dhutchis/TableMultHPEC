...
Sparse arithmetic is impossible without these conditions, since in that case zero operands could generate nonzero results.
...
We may apply the $\oplus$ operation immediately after each third-level loop and obtain an element in $\matr{C}$.
...
performed over an order of magnitude worse, taking over 100 seconds to multiply SCALE 11 inputs
...
We accomodate this case by ``transfering seek control'' %over desired row range subsets
from the tablet server to RemoteWriteIterator by encoding range objects in
iterator options. %using the same technique RemoteSourceIterator uses, 
