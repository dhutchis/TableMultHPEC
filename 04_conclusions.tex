...
in contrast to the KDT which is compute-bound. %and memory bandwidth-bound.
Graphulo therefore has less need for code generation techniques on its Java iterator kernels, 
whereas the KDT uses the SEJITS technique~\cite{catanzaro2009sejits} to translate Python kernels into C++ 
for callback by KDT's underlying Combinatorial BLAS library~\cite{bulucc2011combinatorial},
thereby enhancing compute performance at the expense of restricting operator expressiveness to a DSL.
