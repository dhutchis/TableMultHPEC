...
each of which has write locality to a vertical partition of $\matr{C}$ of length $\ceil{N / P}$.
...
Nevertheless, possible design criteria are to select $P$ to minimize passes through $\matr{B}$,
...
better fits Accumulo's iterator model.  The implementation shows excellent single node performance, achieving rates near 400,000 per second, which is the consistent with the single node peak write rate for Accumulo \cite{kepner2014achieving}.
...
developing graph algorithms that use the Graphulo, % atop the Graphulo library,
