...
SpGEMM via the GraphBLAS user-defined elementwise multiplication and addition functions. 
...
Accumulo tables have many similarities to sparse matrices, though a more precise mathematical definition would be Associative Arrays 
...
We therefore prioritize smaller problems that require low latency %when we must choose between them
...
Keys decompose further into 5-tuple consisting of a row, column family, column qualifier, visibility and timestamp.
For simplicity, we focus on a 2-tuple key consisting of a row and column qualifier.
Each entry forms a row in an Accumulo table. Accumulo divides tables into tablets and assigns each tablet to a tablet server.
Accumulo client applications write new entries via BatchWriters and retrieve entries from tablets sequentially via Scanners
